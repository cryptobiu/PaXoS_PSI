...
/* This file was generated automatically with
   ./gen_bb_mul_code 64 2. Don't edit it! */
...
gf2x_mul1 (unsigned long *c, unsigned long a, unsigned long b)
{
   ...
   lo = (A[b >> 62] << 2) ^ A[(b >> 60) & 3];
   hi = lo >> 60;
   lo = (lo << 4) ^ (A[(b >> 58) & 3] << 2) ^ A[(b >> 56) & 3];
   hi = (hi << 4) | (lo >> 60);
   lo = (lo << 4) ^ (A[(b >> 54) & 3] << 2) ^ A[(b >> 52) & 3];
   hi = (hi << 4) | (lo >> 60);
   lo = (lo << 4) ^ (A[(b >> 50) & 3] << 2) ^ A[(b >> 48) & 3];
   hi = (hi << 4) | (lo >> 60);
   lo = (lo << 4) ^ (A[(b >> 46) & 3] << 2) ^ A[(b >> 44) & 3];
   hi = (hi << 4) | (lo >> 60);
   lo = (lo << 4) ^ (A[(b >> 42) & 3] << 2) ^ A[(b >> 40) & 3];
   hi = (hi << 4) | (lo >> 60);
   lo = (lo << 4) ^ (A[(b >> 38) & 3] << 2) ^ A[(b >> 36) & 3];
   hi = (hi << 4) | (lo >> 60);
   lo = (lo << 4) ^ (A[(b >> 34) & 3] << 2) ^ A[(b >> 32) & 3];
   hi = (hi << 4) | (lo >> 60);
   lo = (lo << 4) ^ (A[(b >> 30) & 3] << 2) ^ A[(b >> 28) & 3];
   hi = (hi << 4) | (lo >> 60);
   ...
   tmp = -((a >> 63) & 1);
   tmp &= ((b & 0xeeeeeeeeeeeeeeee) >> 1);
   hi = hi ^ tmp;
   tmp = -((a >> 62) & 1);
   tmp &= ((b & 0xcccccccccccccccc) >> 2);
   hi = hi ^ tmp;
   tmp = -((a >> 61) & 1);
   tmp &= ((b & 0x8888888888888888) >> 3);
   hi = hi ^ tmp;
