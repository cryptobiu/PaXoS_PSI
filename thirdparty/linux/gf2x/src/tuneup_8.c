...
#if 8 == 1
extern void gf2x_mul8(unsigned long *c, unsigned long a, unsigned long b);
#else
extern void gf2x_mul8(unsigned long *c, const unsigned long *a, const unsigned long *b);
#endif
...
int main(int argc, char *argv[])
{
    ...
    a = (unsigned long *) malloc((N + 8) * sizeof(unsigned long));
    b = (unsigned long *) malloc((N + 8) * sizeof(unsigned long));
    c = (unsigned long *) malloc(2 * 8 * sizeof(unsigned long));
    c0 = (unsigned long *) malloc(2 * 8 * sizeof(unsigned long));
    ...
    for (i = 0; i < 10 && i < N; i++) {
        ...
        gf2x_mul(c0, a + i, 8, b + i, 8);
        st -= microseconds();
#if 8 == 1
        gf2x_mul8 (c, a[i & NMASK], b[i & NMASK]);
#else
        gf2x_mul8 (c, a + (i & NMASK), b + (i & NMASK));
#endif
        st += microseconds();
        if (memcmp(c, c0, 2 * 8 * sizeof(unsigned long)) != 0)
          {
            unsigned long i;
            ...
            fprintf (stderr, "\n");
            fprintf (stderr, "gf2x_mul8 gives:\n");
            for (i = 0; i < 2 * 8; i++)
