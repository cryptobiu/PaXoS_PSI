...
#if 5 == 1
extern void gf2x_mul5(unsigned long *c, unsigned long a, unsigned long b);
#else
extern void gf2x_mul5(unsigned long *c, const unsigned long *a, const unsigned long *b);
#endif
...
int main(int argc, char *argv[])
{
    ...
    a = (unsigned long *) malloc((N + 5) * sizeof(unsigned long));
    b = (unsigned long *) malloc((N + 5) * sizeof(unsigned long));
    c = (unsigned long *) malloc(2 * 5 * sizeof(unsigned long));
    c0 = (unsigned long *) malloc(2 * 5 * sizeof(unsigned long));
    ...
    for (i = 0; i < 10 && i < N; i++) {
        ...
        gf2x_mul(c0, a + i, 5, b + i, 5);
        st -= microseconds();
#if 5 == 1
        gf2x_mul5 (c, a[i & NMASK], b[i & NMASK]);
#else
        gf2x_mul5 (c, a + (i & NMASK), b + (i & NMASK));
#endif
        st += microseconds();
        if (memcmp(c, c0, 2 * 5 * sizeof(unsigned long)) != 0)
          {
            unsigned long i;
            ...
            fprintf (stderr, "\n");
            fprintf (stderr, "gf2x_mul5 gives:\n");
            for (i = 0; i < 2 * 5; i++)
