...
namespace osuCrypto
{
    ...
    PrtyMOtSender::~PrtyMOtSender()
    {
    }

    u64 PrtyMOtSender::getBaseOTCount() const
    {
        if (mGens.size())
            return mGens.size();
        else
    ...
    std::unique_ptr<NcoOtExtSender> PrtyMOtSender::split()
    {
        auto* raw = new PrtyMOtSender();
        raw->mCode = mCode;
    ...
    u64 PrtyMOtSender::recvCorrection(Channel & chl)
    {
    ...
    void PrtyMOtSender::check(Channel & chl, block seed)
    {
        if (mMalicious)
        {
            char c;
